...
Open a terminal window, and then navigate to the root of your C: drive.

At the command prompt, to navigate to the root of your C: drive, enter the following command:

code
 cd C:\
To create a new folder named for your RSSFeedReader project, enter the following command:

code
 mkdir TrainingProjects\RSSFeedReader
To navigate to the new project folder, enter the following command:

code
 cd TrainingProjects\RSSFeedReader
To initialize GitHub Spec Kit in the current directory, enter the following command:
...
To open the RSSFeedReader project in Visual Studio Code, enter the following command:

code
 code .
The code . command opens the current directory (RSSFeedReader) in Visual Studio Code.

Use Visual Studio Code’s EXPLORER view to expand the .github and .specify folders.
...
Ensure that GitHub Copilot’s Chat view is open.
...
In the Chat view, to verify that GitHub Spec Kit commands are available, type /speckit
...
Publish your project to a new GitHub repository.
...
To download the stakeholder documents, open the following link in a browser: RSSFeedReader - stakeholder documents.

Open the folder containing the downloaded ZIP file.

Extract the contents of the downloaded ZIP file to a temporary folder, copy the files, and then paste them into the root folder of the RSSFeedReader project.
...
In Visual Studio Code’s EXPLORER view, expand the StakeholderDocuments folder.
...
In the Chat view, to start a constitution workflow using a combination of inline text and stakeholder documents, enter the following command:
...
Monitor GitHub Copilot’s response.
...
Once GitHub Copilot is finished generating the constitution, take a minute to review the suggested edits.
...
If the /speckit.constitution workflow updated files in the templates folder, take a minute to review those updates as well.

To accept the changes to all updated files, select the Keep button in the Chat view.

Save and then close the updated files.

Commit and push the updated files to your Git repository.

For example:

Open Visual Studio Code’s SOURCE CONTROL view.
Enter a commit message like “Updated constitution using stakeholder requirements.”
...
Use Visual Studio Code’s EXPLORER view to examine the spec-template.md and speckit.specify.agent.md files.
...
Use Visual Studio Code’s EXPLORER view to examine the ProjectGoals.md and AppFeatures.md stakeholder documents.
...
Create a summary description of the RSS Feed Reader app based on the stakeholder documents.
...
Close any files that you have open in the editor.

Ensure that the Chat view is open.
...
In the Chat view, to start a specification workflow that generates a spec.md file using information from your stakeholders document, enter the following command:
...
Monitor GitHub Copilot’s response and provide assistance as needed.
...
Once the specify workflow is complete, use Visual Studio Code’s EXPLORER view to expand the specs and checklists folders.

In the EXPLORER view, select spec.md, and then take a couple minutes to review the spec.md file.
...
Verify that the User Stories (and Acceptance Scenarios) in the spec.md file are specific and testable:
...
Verify that the Requirements section of the spec.md file includes key requirements from your stakeholder requirements document.
...
In the EXPLORER view, select requirements.md, and then take a minute to review the requirements.md file.
...
Accept the suggested file updates, and then save the spec.md and requirements.md files.

Commit the specification files and publish the new branch to your Git repository.
...
Use Visual Studio Code’s EXPLORER view to open the plan-template.md and speckit.plan.agent.md files.

Take a minute to review the plan-template.md and speckit.plan.agent.md files.
...
In the Chat view, to start a workflow that generates the plan.md file, enter the following command:

code
 /speckit.plan --files StakeholderDocuments/ProjectGoals.md StakeholderDocuments/TechStack.md
Monitor GitHub Copilot’s response and provide assistance in the Chat view.
...
Once the plan workflow is complete, verify that the following files were added to the root of the specs folder:
...
Take a few minutes to review the research.md, plan.md, quickstart.md, and data-model.md files.
...
After reviewing the files, accept all of the suggested edits.
...
Save the files, and then commit and sync your changes.
...
Use Visual Studio Code’s EXPLORER view to open the tasks-template.md and speckit.tasks.agent.md files.

Take a minute to review the tasks-template.md and speckit.tasks.agent.md files.
...
In the Chat view, to start generating the tasks.md file, enter the following command:
...
Once the tasks workflow is complete, take a few minutes to review the tasks.md file.
...
Accept the suggested file updates, and then save the tasks.md file.

Commit the changes and then sync the updates.
...
Open the tasks.md file, locate the Implementation Strategy section, and then review the suggested MVP strategy.
...
In the Chat view, enter a command that starts the implement workflow for the MVP First strategy:
...
Continue monitoring the implementation workflow until all tasks required for the MVP application are complete.

GitHub Copilot should notify in the Chat view when the backend and frontend applications are complete.

Accept all changes made to the project files.
...
Start the backend application, and then start the frontend application.
...
Verify that the frontend application opens successfully in the browser.
...
You start the backend application. The backend application is running locally on http://localhost:5151.
You start the frontend application. The frontend application is running locally on http://localhost:5213.
You open a browser and navigate to the URL specified for the frontend application.
When the page loads, it reports an error: “An unhandled error has occurred. Reload”.
You select the “Reload” option, but the error persists.
You open the browser’s developer tools console to investigate further.
In the developer tools console, you see the following error message:
...
You report the issue to GitHub Copilot in the Chat view.
...
code
 ...
 at Microsoft.AspNetCore.Components.Routing.Router.<>c__DisplayClass82_0.<RunOnNavigateAsync>b__1(:5213/RenderTreeBuilder builder)
 at Microsoft.AspNetCore.Components.Rendering.ComponentState.RenderIntoBatch(:5213/RenderBatchBuilder batchBuilder, RenderFragment renderFragment, Exception& renderFragmentException)".
GitHub Copilot analyzes the information you provided and begins debugging the issue.
...
Take a couple minutes to verify that the frontend application is working as expected.
...
Given no subscriptions have been added, When the user loads the page, Then an empty state is shown (for example, “No subscriptions yet” message)
Given the subscription management interface is loaded, When the user enters a valid feed URL in the input field and clicks “Add Subscription”, Then the system accepts the URL and confirms the subscription was added
Given the user has entered a feed URL, When the user submits the form, Then the input field is cleared and ready for another URL
Given the user enters an empty string or whitespace-only input, When they attempt to add the subscription, Then the system prevents submission (basic client-side validation)
Given the user has added one subscription, When the page displays, Then the subscription URL is visible in the list
Given the user has added multiple subscriptions, When the page displays, Then all subscription URLs are visible in the list in the order they were added (newest last)
...
Continue testing the frontend (and reporting any issues to GitHub Copilot) until all acceptance scenarios for the MVP application pass successfully.
