...
describe("firstTouchStrategy", () => {
	it("assigns 100% credit to the first touchpoint before conversion", () => {
		...
		const results = firstTouchStrategy.attribute(baseConversion, touchpoints);
		...
		expect(results[0].touchpointId).toBe(1);
	});

	it("marks as unattributed when no touchpoints exist", () => {
		const results = firstTouchStrategy.attribute(baseConversion, []);
		expect(results).toHaveLength(1);
	...
	it("uses alphabetical tie-breaking for same timestamps", () => {
		...
		const results = firstTouchStrategy.attribute(baseConversion, touchpoints);
		// Sorted alphabetically: Email, Social. First = Email
